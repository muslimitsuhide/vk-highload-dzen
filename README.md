...
- В качестве основного хранилища данных будет взята СУБД **PostgreSQL** кроме *session*. Так как она является довольно гибким инструментом для работы с разными типами данных. 
...
- Таблицы post, article, video содержат посты, статьи и видео, созданные пользователями. Шардинг можно применить здесь так же по полю, связанному с пользователем **user_id**.
...
- Таблицы **post, article, video**: репликация этих таблиц может быть полезна для обеспечения отказоустойчивости и улучшения производительности чтения. Данные этих таблиц часто запрашиваются на чтение, наличие реплик позволит распределить нагрузку и обеспечить быстрый доступ к данным для пользователей.

- Таблица user: репликация таблицы пользователей может быть полезной для повышения доступности и обеспечения сбалансированного распределения запросов на аутентификацию и авторизацию пользователей.
