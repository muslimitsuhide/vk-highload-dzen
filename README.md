...
- В качестве основного хранилища данных будет взята СУБД **PostgreSQL** кроме *session*. Так как она является довольно гибким инструментом для работы с разными типами данных и имеет тип данных JSONB (бинарное представление данных в формате JSON), что нам необходимо для хранения данных, представляющих собой массив элементов (как пример, это поля url, email и phone_numbur в базе channel, в которых может быть указано как и 0 элементов, так и массив из 10). 
...
- Таблицы **post, article, video**: репликация этих таблиц может быть полезна для обеспечения отказоустойчивости и улучшения производительности чтения.
...
## Часть 11. Обеспечение надежности <a name="11"></a>
...
В Дзене основной функционал - это авторизация, регистрация и просмотр ленты и если у нас отвалится возможность поставить лайк, написать комментарий или даже открыть саму публикацию, то не упадет вся страница с лентой, а продолжит работать за счет Graceful degradation.
