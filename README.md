...
- Отказ компонента рекомендательной системы: 
  ...
  Если сервис, отвечающий за рекомендации (Recomender) контента пользователям перестает работать, то приложение переходит в состояние Graceful degradation и мы начинаем использовать альтернативный метод показа ленты: идем в БД Subscription, смотрим, на кого подписан пользователь и достаем последние публикации этих каналов и отдаем в ленте пользователю.

- Отказ Elasticsearch:

  При отказе Elasticsearch приложение переходит в состояние Graceful degradation и поиск начинает работать альтернативным методом: идем в БД Channel и делаем поиск по полю title.
