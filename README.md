...
|         advertiser           |Данные рекламодателя, которые заполняются при регистрации и дальнейшего заполнения профиля|
...
|         ad        |Данные рекламы, которые заполняются при создании|
...
Рекламодатель создал рекламную кампанию, указав сайт, где продает бетон(как пример), ключевое слово: *бетон*, заголовок, описание, фотографию и указал ставку в 100₽ за клик по его банеру. Все факторы вместе со статистикой (если этот банер уже крутился) подаются на вход MatrixNet^[11] (модель на вход которой мы подаем набор факторов, а на выходе получаем число, в данном случае, это обобщенное качество нашего баннера). 
...
[^11]: [Рекламные трафареты в поиске аукционы и алгоритмы](https://www.youtube.com/watch?v=wyDy_Dms51w&ab_channel=YandexforDevelopers)

[^10]: [Технологии. Матрикснет](https://yandex.ru/company/technologies/matrixnet/)